...
export default function Hero({ onSearch }) {
  ...
  return (
    <section className="flex flex-col items-center justify-center text-center py-20 px-6 bg-gray-50">
      ...
      {/* Formulário de busca */}
      <form onSubmit={handleSubmit} className="flex w-full max-w-2xl bg-white shadow-lg rounded-lg overflow-hidden mb-4">
        <input
          type="text"
          placeholder="Digite cidade, bairro ou código do imóvel..."
          value={query}
      ...
      <div className="flex gap-4 mt-4 flex-wrap justify-center">
        {["casa", "apartamento", "terreno", "chacara", "sitio"].map((cat) => (
          <button
            key={cat}
            onClick={() => setSelectedCategory(cat)}
            className={`px-4 py-2 rounded-full border ${
              selectedCategory === cat ? "bg-blue-600 text-white" : "bg-white text-gray-800 border-gray-300"
